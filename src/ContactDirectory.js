...
function ContactDirectory({ onCallContact, onVideoCallContact, isTransferMode, isAddMode, setShouldInitiateCall, handleTransferClick, t, token, title, isAdmin, refreshTrigger, refreshContacts  }) {
  ...
  useEffect(() => {
    const fetchContacts = async () => {
      ...
      try {
        ...
        const contactsWithStatus = data.map(user => ({
          ...
          is_internal: user.is_internal || 'externe',
          profile_picture: user.profile_picture || defaultPhoto
    ...
    fetchContacts();
  }, [token, t, refreshContacts]); // Ajoutez refreshContacts ici
  ...
  const highlightText = (text, highlight) => {
   ...
   const handleProfileClick = async (contact) => {
    try {
      const response = await fetch(`http://192.168.1.95:3000/users/${contact.extension}`, {
        headers: {
          'Authorization': `Bearer ${token}`,
        },
      });
      ...
      setSelectedProfile({
        ...fullProfile,
        name: `${fullProfile.prenom || ''} ${fullProfile.nom || ''}`.trim(),
        extension: fullProfile.extension,
        profile_picture: fullProfile.profile_picture || defaultPhoto,
      });
    } catch (error) {
